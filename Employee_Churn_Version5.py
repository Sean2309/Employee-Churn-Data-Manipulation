...
parser.add_argument("config_file", nargs="?", const="config.json", default="config.json",type=str, help="name of config file")
...
with open(args.config_file) as config_file:
    ...
    cols_w_eval = data["list_of_val_cols_calc"]
    cols_ = data["list_of_val_cols_nocalc"]
...
def hist_plot(viz_df: pd.DataFrame, x_label: str, hue_label: str, y_label: str):
    hist_fig = sns.displot(data=viz_df, x=x_label, y=y_label, kind="hist", hue=hue_label, palette="bright", height=4)
...
def kde_plot(viz_df: pd.DataFrame, x_label: str, hue_label: str, y_label: str):
    kde_fig = sns.displot(data=viz_df, x=x_label, y=y_label, kind="kde", hue=hue_label, palette="bright", height=4) 
    x_label = str_replacing(x_label)
    y_label = str_replacing(y_label)
    hue_label = str_replacing(hue_label)
...
initialisation(boolean, df, mapping)
#initialisation(boolean, df, mapping, pri_cols, sec_col, list_of_val_cols_calc, list_of_val_cols_nocalc)